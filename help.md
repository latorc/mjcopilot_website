...
## 使用帮助
### 游戏客户端
软件使用中间人 (MITM) 代理获取游戏客户端和服务器之间传输的信息。推荐使用内置的浏览器进行游戏。
...
4. 网页客户端最小化或者标签处于后台时，将无法操作自动打牌。Windows 系统中，如果想隐藏窗口，可以使用 Win+Tab 创建虚拟桌面，并将窗口放到虚拟桌面中。
5. 如果使用其他客户端，可以用代理软件 (如 Proxifier) 使客户端流量通过代理`http://localhost:{mitm端口}`访问。其他客户端暂不支持自动打牌。
...
8. 状态栏中，显示模型信息，以及主程序和浏览器状态（括号中显示刷新率，作为性能参考；通常数值在 100 以上说明程序运行流畅；进行自动打牌操作时，浏览器刷新率会下降），以及游戏运行状态。如果程序或 AI 发生错误，状态栏中会显示错误信息。

![GUI](\assets\images\shot1.png){: height="50"} 

### 设置项说明
1. 自动启动浏览器：程序启动时，自动启动浏览器和游戏。
2. 客户端大小：浏览器中客户端的分辨率。
3. 雀魂网址：浏览器中打开雀魂的网址。
4. MITM服务端口和上游代理：见前述 MITM 帮助。
