...
9. “启用浏览器插件”：勾选后内置浏览器启动时将加载插件。将 Chrome 插件文件（.crx）用压缩工具解压到单独的目录，并将该目录放到程序目录 `chrome_ext` 中。例如，使用油猴插件，将油猴插件的 .crx 文件内容解压缩到 `chrome_ext\TamperMonkey\` 目录中即可。使用插件有风险，请自行甄别。
